// 🔧 Fix Vercel All Dependencies Script
// This script will help fix the build errors due to all missing dependencies
  ...
  if (checkFileExists(uiComponentsPath)) {
    ...
    for (const component of uiComponents) {
      if (component.endsWith('.tsx')) {
        ...
        // Find all imports from @radix-ui
        const radixImports = componentContent.match(/from ["'](@radix-ui\/[^"']+)["']/g);
        
        if (radixImports) {
          for (const importStatement of radixImports) {
            const match = importStatement.match(/from ["'](@radix-ui\/[^"']+)["']/);
            if (match) {
...
function updatePackageJson() {
  log('📦 Updating package.json with all missing dependencies...', 'blue');
  ...
  log('✓ package.json updated with all missing dependencies', 'green');
}
...
function createDeploymentInstructions() {
  ...
  const instructions = `
# 🔧 Fix Vercel All Dependencies Instructions
...
1. **Update Dependencies**:
   - Run the fix-vercel-all-dependencies.js script
...
`;
  ...
  fs.writeFileSync('VERCEL-ALL-DEPENDENCIES-FIX-INSTRUCTIONS.md', instructions);
  log('✓ Deployment instructions created', 'green');
}

function main() {
  log('🔧 Fix Vercel All Dependencies Script', 'bright');
  log('=================================', 'bright');
  log('');
  
  log('🔍 Fixing all missing dependencies...', 'blue');
  ...
  log('', 'reset');
  log('✅ All missing dependencies fixed successfully', 'green');
  log('', 'reset');
  ...
  log('6. Check VERCEL-ALL-DEPENDENCIES-FIX-INSTRUCTIONS.md for detailed solutions', 'blue');
  log('', 'reset');
