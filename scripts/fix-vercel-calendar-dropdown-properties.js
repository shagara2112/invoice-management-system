// 🔧 Fix Vercel Calendar Dropdown Properties Script
// This script will help fix the dropdown properties in calendar component
...
function fixCalendarComponent() {
  log('🔧 Fixing calendar component dropdown properties...', 'blue');
  ...
  if (checkFileExists(calendarPath)) {
    ...
    const fixedContent = calendarContent
      ...
      .replace(
        /dropdowns: cn\("w-full flex items-center text-sm font-medium justify-center h-\(--cell-size\) gap-1\.5"\),/g,
      ...
      .replace(
        /dropdown: cn\("relative has-focus:border-ring border border-input shadow-xs has-focus:ring-ring\/50 has-focus:ring-\[3px\] rounded-md"\),/g,
      ...
      // Replace dropdown_container with dropdown
      .replace(
        /dropdown_container: cn\("absolute bg-popover inset-0 opacity-0"\),/g,
        'dropdown: cn("absolute bg-popover inset-0 opacity-0"),'
      );
    
    // Now we need to merge all dropdown properties into one
    const mergedContent = fixedContent
      .replace(
        /dropdown: cn\("w-full flex items-center text-sm font-medium justify-center h-\(--cell-size\) gap-1\.5"\),\s*dropdown: cn\("relative has-focus:border-ring border border-input shadow-xs has-focus:ring-ring\/50 has-focus:ring-\[3px\] rounded-md"\),\s*dropdown: cn\("absolute bg-popover inset-0 opacity-0"\),/g,
    ...
    fs.writeFileSync(calendarPath, mergedContent);
    log('✓ Calendar component dropdown properties fixed', 'green');
  } else {
...
function createDeploymentInstructions() {
  ...
  const instructions = `
# 🔧 Fix Vercel Calendar Dropdown Properties Instructions

## Common Causes and Solutions:

### 1. Calendar Component Dropdown Properties Issues
- **Problem**: Build process fails due to incorrect dropdown properties in classNames object
- **Solution**: 
...
- **Issue**: Property 'dropdowns' does not exist in type 'Partial<StyledElement<string>>'
- **Fix**: Replace dropdowns with dropdown
- **Issue**: Duplicate dropdown properties
- **Fix**: Merge all dropdown properties into one
...
1. **Update Calendar Component**:
   - Run the fix-vercel-calendar-dropdown-properties.js script
   - Fix all calendar component dropdown properties issues
...
`;
  ...
  fs.writeFileSync('VERCEL-CALENDAR-DROPDOWN-PROPERTIES-FIX-INSTRUCTIONS.md', instructions);
  log('✓ Deployment instructions created', 'green');
}

function main() {
  log('🔧 Fix Vercel Calendar Dropdown Properties Script', 'bright');
  log('=============================================', 'bright');
  log('');
  ...
  log('', 'reset');
  log('✅ Calendar component dropdown properties fixed successfully', 'green');
  log('', 'reset');
  ...
  log('6. Check VERCEL-CALENDAR-DROPDOWN-PROPERTIES-FIX-INSTRUCTIONS.md for detailed solutions', 'blue');
  log('', 'reset');
