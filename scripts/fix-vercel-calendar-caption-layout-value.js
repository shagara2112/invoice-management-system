// 🔧 Fix Vercel Calendar Caption Layout Value Script
// This script will help fix the caption layout comparison in calendar component by using string value
...
function fixCalendarComponent() {
  log('🔧 Fixing calendar component caption layout value...', 'blue');
  ...
  if (checkFileExists(calendarPath)) {
    ...
    const fixedContent = calendarContent
      // Remove CaptionLayout from the import statement
      .replace(
        /import { DayPicker, CaptionLayout } from "react-day-picker"/g,
        'import { DayPicker } from "react-day-picker"'
      )
      // Fix the caption layout comparison
      .replace(
        /caption_label: cn\("select-none font-medium", captionLayout === CaptionLayout\.Label \? "text-sm" : "rounded-md pl-2 pr-1 flex items-center gap-1 text-sm h-8 \[&>svg\]:text-muted-foreground \[&>svg\]:size-3\.5"\),/g,
    ...
    log('✓ Calendar component caption layout value fixed', 'green');
  } else {
...
function createDeploymentInstructions() {
  ...
  const instructions = `
# 🔧 Fix Vercel Calendar Caption Layout Value Instructions

## Common Causes and Solutions:

### 1. Calendar Component Caption Layout Value Issues
- **Problem**: Build process fails due to incorrect caption layout comparison
- **Solution**: 
  1. Remove CaptionLayout from the import statement
...
- **Issue**: 'CaptionLayout' only refers to a type, but is being used as a value here
- **Fix**: Remove CaptionLayout from the import statement and use "label" instead of CaptionLayout.Label
...
1. **Update Calendar Component**:
   - Run the fix-vercel-calendar-caption-layout-value.js script
   - Fix all calendar component caption layout value issues
...
`;
  ...
  fs.writeFileSync('VERCEL-CALENDAR-CAPTION-LAYOUT-VALUE-FIX-INSTRUCTIONS.md', instructions);
  log('✓ Deployment instructions created', 'green');
}

function main() {
  log('🔧 Fix Vercel Calendar Caption Layout Value Script', 'bright');
  log('==============================================', 'bright');
  log('');
  ...
  log('', 'reset');
  log('✅ Calendar component caption layout value fixed successfully', 'green');
  log('', 'reset');
  ...
  log('6. Check VERCEL-CALENDAR-CAPTION-LAYOUT-VALUE-FIX-INSTRUCTIONS.md for detailed solutions', 'blue');
  log('', 'reset');
