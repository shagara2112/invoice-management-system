// 🔧 Fix Vercel Calendar Week Number Script
// This script will help fix the week number properties in calendar component
...
function fixCalendarComponent() {
  log('🔧 Fixing calendar component week number properties...', 'blue');
  ...
  if (checkFileExists(calendarPath)) {
    ...
    // Fix the week number properties
    const fixedContent = calendarContent
      // Replace week_number_header with weeknumber
      .replace(
        /week_number_header: cn\("select-none w-\(--cell-size\)"\),/g,
        'weeknumber: cn("select-none w-(--cell-size)"),'
      )
      // Replace week_number with weeknumber
      .replace(
        /week_number: cn\("text-\[0\.8rem\] select-none text-muted-foreground"\),/g,
        'weeknumber: cn("text-[0.8rem] select-none text-muted-foreground"),'
      );
    
    fs.writeFileSync(calendarPath, fixedContent);
    log('✓ Calendar component week number properties fixed', 'green');
  } else {
...
`;
  ...
  fs.writeFileSync('VERCEL-CALENDAR-WEEK-NUMBER-FIX-INSTRUCTIONS.md', instructions);
  log('✓ Deployment instructions created', 'green');
}

function main() {
  log('🔧 Fix Vercel Calendar Week Number Script', 'bright');
  log('==========================================', 'bright');
  log('');
  ...
  log('', 'reset');
  log('✅ Calendar component week number properties fixed successfully', 'green');
  log('', 'reset');
  ...
  log('6. Check VERCEL-CALENDAR-WEEK-NUMBER-FIX-INSTRUCTIONS.md for detailed solutions', 'blue');
  log('', 'reset');
